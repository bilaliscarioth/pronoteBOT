...
module.exports = client => {
	...
	client.pronoteRequest = async (url, login,mdp, userChoice, exampleEmbed, id) => {
		if(login == null && mdp == null) return "Aucun identifiant..."
		var session = client.getStudent(id);
		if(session){
			if(userChoice == "login"){ client.username = login; client.mdp = mdp; return message.reply("Vous êtes bien connecté" +
				...
				for(var i = 0; i < marks.subjects.length; i++){
					...
					if(noteMatiere) exampleEmbed.addField(` Notes en ${nomMatiere} -> ${marks.subjects[i].averages.student} /` +
				...
				for(var time in marks) exampleEmbed.addField(`**${time.subject} - ${time.teacher}**`, `=> **${time.room}**`, false);	
			...
			session = await pronote.login(url, login, mdp);
			client.newStudent(id, session);
			return client.pronoteRequest(url, login, mdp, userChoice, exampleEmbed, id);
		}
		return exampleEmbed;
   }
   ...
   client.newStudent = (id, session) => {
		client.listeSession.set(id, session);
		console.log(Object(client.listeSession).keys());
   }
   client.getStudent = (id) => {
